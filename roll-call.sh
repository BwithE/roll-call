    ...
    for service in $services; do 
        ...
        case $service in
            http|https|http?)
                ...
                echo -e "${BLUE}[+] Starting http enumeration with gobuster/dirb/nikto/whatweb on Port: $i${NC}"
            ...
            smtp|smtp?|mail)
                sleep 1
                echo -e "${BLUE}[+] Starting SMTP enumeration on Port: $i${NC}"
...
echo -e "${ORANGE}[+] Waiting on the background scans to finish. Press the ENTER/RETURN key to check progress.${NC}"
